...
############################# for testing the number of image get recieved ##################################
...
def recognition():
    ...
    return render_template('getImage4.html', my_dict=my_dict)
...
def image():
    ...
    f = str(counter.count) + '.jpeg'
    counter.count += 1
    # f = str(randint(0, 1000)) + '.jpeg
    temp_path = PATH_TO_TEST_IMAGES_DIR + "/" + image_check_name
    i.save('%s/%s' % (temp_path, f))
    # print(counter.count)
    if counter.count > 14:
        try:
            if "image_check_name" in session:
            ...
            flag, empcde, status_flag, blink = facerecognitionFunction(f, branch_code, ip, lat, lon, image_check_name)
            counter.count += 1
            ...
            if flag == 1 and status_flag == 1 and blink > 1:
                msg = 'face_recognized/' + empcde
                # print("Execution time : " + str((time.time() - startTime)))
                # print('Image count: {}'.format(counter.count))
                counter.count = 0
                return msg
            #elif device == 1:
                #return 'device_found'
        ...
        return "retake_image_from_webcam"
...
def test():
    ...
    try:
        os.mkdir(PATH_TO_TEST_IMAGES_DIR + '/' + session["image_check_name"])
    except FileExistsError:
        pass
    select_officecode = conn.cursor()
    select_officecode.execute("select * from VW_PYCODMAS")
